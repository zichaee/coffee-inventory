...
export async function onRequestPost(context) {
  ...
  if (permissions.access_inventory_write == 1) {
    const ps = context.env.INVENTORY_MANAGEMENT.prepare("update inventory set received_date = ?, expiration_date = ?, quantity = ?, storage_location = ?, unit_price = ?, note = ? where product_id = ?;")
      .bind(...values);
